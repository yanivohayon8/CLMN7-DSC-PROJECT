...
def extract_time_titles_changed(listi, wanted_similarity_percent):
    wantedTimes=[]
    index = 0
    ...
    while index < len(listi):
        ...
        if text:
            ...
            if fuzz.ratio(text.lower(),last_text.lower()) > wanted_similarity_percent:
...
def get_time_titles_changed_with_downlode(video_url, target_path, wanted_frequency, fileName, wanted_similarity_percent ):
    print(video_url)
    ...
    print(extract_time_titles_changed(arr, wanted_similarity_percent))

def get_time_titles_changed(video_path, target_path, wanted_frequency, video_name, wanted_similarity_percent):
    images = extract_images_from_video(video_path, target_path, frequency = wanted_frequency, name=video_name, max_images=1500, silent=False)
    return extract_time_titles_changed(images, wanted_similarity_percent)
