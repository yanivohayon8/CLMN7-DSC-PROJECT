...
def getSubjSilentRanges(wav_file_name, silence_threshold, silence_length, step_size):
    SLICE_LEN = silence_length
    STEP_SIZE = step_size
    silence_thresh = silence_threshold
        
    switchPoints = []
    audio = AudioSegment.from_wav('./../../data/raw/audio/' + wav_file_name)
	if audio != null :
...
def downloadAudioFromYoutube(youtube_path):
	ydl_opts = {
		...
		'postprocessors': [{
			...
			'outtmpl': './../../data/raw/audio/%(title)s.%(ext)s',
		}],
	}
